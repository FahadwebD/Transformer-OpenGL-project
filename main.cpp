...
void background()
{
    ...
    glVertex3i(-2400,0,0); //Specifies the position of a vertex in 2, 3, or 4 dimensions. The coordinates can be specified as shorts s, ints i, floats f or doubles d. If v is present, the argument is a pointer to an array containing the coordinates.
...
void robot()
{
    ...
    //Robot Left Knee
    glColor3f(0.2,0.2,0.2); //Sets the present RGB (or RGBA) colors. Valid types are byte (b),int (i), float (f), double (d), unsigned byte (ub), unsigned short (us), and unsigned int (ui).
...
void vehicle()
{
    ...
    if(vflag==1)
    {
        ...
        //Vehicle Hood
        glColor3f(1,0.639,0.101); //Sets the present RGB (or RGBA) colors. Valid types are byte (b),int (i), float (f), double (d), unsigned byte (ub), unsigned short (us), and unsigned int (ui).

        glBegin(GL_QUADS); //Initiates a new primitive of type mode and stars the collection of vertices. Values of mode include GL_POINTS, GL_LINES, and GL_POLYGON.
